    ...
    <table class="table table-bordered text-center align-middle bg-white shadow-sm">
        <thead class="bg-label-warning">
            <tr>
                ...
                <th><?= $p->period_index ?><br><small class="text-muted"><?= substr($p->start_time, 0, 5) ?></small></th>
                <?php endforeach; ?>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <?php foreach($periods as $p): 
                    $isLocked = isset($lock_map[$d->day_index . '_' . $p->period_index]);
                ?>
                <td class="p-1 slot-toggle" style="height: 60px; cursor: pointer;" 
                    onclick="toggleRoomLock(this, '<?= $room_name ?>', '<?= $d->day_index ?>', '<?= $p->period_index ?>')">
