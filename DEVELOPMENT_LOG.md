...
## [2026-05-19] - ระบบสถิติเด็กจบแล้วไปไหนและปรับปรุงระบบจัดเก็บข้อมูลนักเรียนแนะแนว (Graduate Destination Statistics & Academic Admissions Refinement) 🟢📅✨

### 🚀 ฟีเจอร์ที่เพิ่ม/แก้ไข (Added/Modified)
- **ระบบรายงานและบันทึกข้อมูลสถิติผู้สำเร็จการศึกษา (Fully Database-Driven Graduate Destination Statistics Dashboard):**
    - ลบลิงก์เมนู "ข้อมูลเยี่ยมบ้าน" (Home Visit) ออกจากเมนูหลักนำทางด้านซ้าย (Sidebar Menu) ใต้หัวข้อ "งานแนะแนว" ตามนโยบายการจัดลำดับเนื้อหาของโรงเรียน
    - เพิ่มลิงก์ระบบใหม่ **"สถิติเด็กจบแล้วไปไหน"** (Graduate Destinations Statistics) ชี้เป้าไปที่เส้นทางควบคุมใหม่ `Admin/Acade/Guidance/GraduationStats`
    - สร้างหน้าแสดงรายงานแดชบอร์ดสถิต�    - **โมดอลบันทึกและแก้ไขข้อมูลแบบเรียลไทม์ (Interactive Graduation Route Recording Modal):**
        - พัฒนาโมดอลบันทึกข้อมูลระดับพรีเมียม `EditGraduationModal` สำหรับให้แอดมินหรือเจ้าหน้าที่แนะแนวกรอกบันทึกข้อมูลและเส้นทางชีวิตของนักเรียนแต่ละคนได้โดยตรงแบบเรียลไทม์
        - ออกแบบช่องกรอกข้อมูลให้ปรับเปลี่ยนตามสถานะโดยอัตโนมัติ (Dynamic Input Fields): แสดงช่องกรอก "ชื่อสถาบันการศึกษา / มหาวิทยาลัย (คณะ/สาขา)" เมื่อเลือกสถานะ "ศึกษาต่อ" และแสดงช่องกรอก "ชื่อสถานที่ทำงาน / บริษัท (ตำแหน่งงาน/อาชีพ)" เมื่อเลือกสถานะ "ทำงาน"
        - **เพิ่มฟีเจอร์ปุ่มด่วนสำหรับกรอกข้อมูลสถาบันการศึกษาและอาชีพยอดนิยม (Quick Select Buttons / Tags Integration):**
            - ติดตั้งแผงปุ่มด่วนรูปแบดจ์โค้งมน (Pill Badges) ใต้ช่องกรอก เพื่อให้เจ้าหน้าที่สามารถเลือกใส่รายชื่อมหาวิทยาลัยและเส้นทางชีวิตยอดนิยมในประเทศไทยได้ด้วยการคลิกเพียงครั้งเดียว (เช่น ม.เกษตรศาสตร์, จุฬาลงกรณ์ฯ, ม.ธรรมศาสตร์, มศว, ม.มหิดล หรือเส้นทางชีวิตอาชีพ เช่น ธุรกิจส่วนตัว/ค้าขาย, รับราชการ, พนักงานบริษัทเอกชน, อาชีพอิสระ, ค้าขายออนไลน์) ช่วยลดขั้นตอนการพิมพ์และเพิ่มความแม่นยำในการบันทึกข้อมูลอย่างมหาศาล
        - **ระบบแก้ไขปีการศึกษาที่สำเร็จการศึกษาแบบพลวัตในหน้าต่างเดียวกัน (Dynamic Graduation Year Editing Integration):**
            - พัฒนาช่องอินพุตตัวเลข **"ปีการศึกษาที่สำเร็จการศึกษา (พ.ศ.)"** (`modalYearFinish`) เพิ่มเติมเข้าไปในหน้าต่างป๊อปอัปเดียวกัน ทำให้เจ้าหน้าที่สามารถแก้ไข ตรวจสอบ หรือย้ายปีการศึกษาจบของนักเรียนได้ทันทีในหน้าจอเดียวโดยไม่ต้องสลับเข้าสู่หน้าต่างประวัตินักเรียนปกติ
            - ปรับปรุงการรับค่าข้ามเซิร์ฟเวอร์แบบ **Double Database Update** ในเมธอด `AdminReportGraduationStatsSave` ของคอนโทรลเลอร์ `ConAdminReportResult.php` ให้ส่งข้อมูลบันทึกข้ามสิทธิ์และอัปเดตฟิลด์ `YearFinish` ในตาราง `tb_students` (ฐานข้อมูลวิชาการกลาง) ควบคู่ไปกับการอัปเดตรายละเอียดเส้นทางชีวิตในฐานข้อมูลทะเบียนประวัติครูและนักเรียน โดยระบบจะทำการกรองและทำความสะอาดเลขบัตรประชาชน (`stu_iden` / `StudentIDNumber`) ให้ตรงกัน 100%
            - เชื่อมต่อการนำเข้าข้อมูลและการอ้างอิงผ่าน Attribute `data-year-finish` บนปุ่มแก้ไข และผูกตรรกะ JavaScript ให้โหลดแสดงผลรวมกับแดชบอร์ดสถิติในรูปแบบ Real-time
        - **ระบบเชื่อมต่อ API ข้อมูลสถิติเด็กจบการศึกษาข้ามระบบ (Graduate Statistics REST/JSON API Endpoint & Public Dashboard):**
            - พัฒนาเส้นทางและสร้าง API Endpoint ใหม่อย่างเป็นทางการที่หน้าต่างระบุบที่อยู่ `/admin/academic/api` (รองรับการทำงานทั้งคำขอแบบ GET และ POST)
            - **การทำงานแบบไฮบริด (Hybrid Client Detection):**
              1. **โหมดสาธารณะบุคคลภายนอก (Public Guest Dashboard):** หากบุคคลภายนอก (ผู้ปกครอง, คณะกรรมการสถานศึกษา หรือนักเรียน) เข้าลิงก์นี้โดยตรงผ่านทางเว็บเบราว์เซอร์ (`GET` request) ระบบจะตรวจจับและเรนเดอร์หน้าเว็บแดชบอร์ดสถิติผู้สำเร็จการศึกษาฉบับสาธารณะสุดหรูหรา (`AdminReportGraduationStatsPublic.php`) ที่เป็นแบบ **อ่านอย่างเดียว (Read-Only 100%)** ปิดการทำงานของปุ่มดำเนินการ/แก้ไข/หน้าต่างป๊อปอัปทั้งหมดเพื่อความปลอดภัยสูงสุด แต่ยังคงความสามารถในการฟิลเตอร์ กราฟวงกลม และการค้นหาแบบไลฟ์เสิร์ชและปุ่มส่งออก Excel
              2. **โหมดส่งกลับ JSON (RESTful JSON Feed):** หากถูกดึงผ่านสคริปต์ AJAX/API หรือระบุพารามิเตอร์ปีการศึกษา `?year=...` หรือแนบ `?format=json` ระบบจะส่งข้อมูลอาเรย์สถิติอย่างเป็นทางการกลับไปในรูปแบบ JSON เพื่ออำนวยความสะดวกในการนำข้อมูลสถิติเด็กจบไปเชื่อมต่อแสดงผลภายนอกโรงเรียน
        - เชื่อมโยง API และเส้นทาง POST `/GraduationStats/Save` ไปยังคอนโทรลเลอร์ เพื่อนำเข้าและอัปเดตข้อมูลตรงไปยังฟิลด์ `stu_future_education` และ `stu_career_interest` ในตาราง `skjacth_personnel.tb_students` โดยอ้างอิงและทำความสะอาดเลขบัตรประชาชน (`stu_iden`) ป้องกันความผิดพลาดของระเบียน
        - บูรณาการ JavaScript AJAX และ SweetAlert2 เพื่อทำการบันทึกอย่างรวดเร็วโดยไม่ต้องรีโหลดหน้าเว็บ (Single Page UX) พร้อมทั้งอัปเดตแผนภูมิวงกลมและตัวเลขสถิติบนแดชบอร์ดทันทีที่ทำรายการสำเร็จวกในการนำข้อมูลสถิติเด็กจบไปเชื่อมต่อแสดงผลภายนอกโรงเรียน
        - เชื่อมโยง API และเส้นทาง POST `/GraduationStats/Save` ไปยังคอนโทรลเลอร์ เพื่อนำเข้าและอัปเดตข้อมูลตรงไปยังฟิลด์ `stu_future_education` และ `stu_career_interest` ในตาราง `skjacth_personnel.tb_students` โดยอ้างอิงและทำความสะอาดเลขบัตรประชาชน (`stu_iden`) ป้องกันความผิดพลาดของระเบียนพยอดนิยม (Quick Select Buttons / Tags Integration):**
            - ติดตั้งแผงปุ่มด่วนรูปแบดจ์โค้งมน (Pill Badges) ใต้ช่องกรอก เพื่อให้เจ้าหน้าที่สามารถเลือกใส่รายชื่อมหาวิทยาลัยและเส้นทางชีวิตยอดนิยมในประเทศไทยได้ด้วยการคลิกเพียงครั้งเดียว (เช่น ม.เกษตรศาสตร์, จุฬาลงกรณ์ฯ, ม.ธรรมศาสตร์, มศว, ม.มหิดล หรือเส้นทางชีวิตอาชีพ เช่น ธุรกิจส่วนตัว/ค้าขาย, รับราชการ, พนักงานบริษัทเอกชน, อาชีพอิสระ, ค้าขายออนไลน์) ช่วยลดขั้นตอนการพิมพ์และเพิ่มความแม่นยำในการบันทึกข้อมูลอย่างมหาศาล
        - **ระบบแก้ไขปีการศึกษาที่สำเร็จการศึกษาแบบพลวัตในหน้าต่างเดียวกัน (Dynamic Graduation Year Editing Integration):**
            - พัฒนาช่องอินพุตตัวเลข **"ปีการศึกษาที่สำเร็จการศึกษา (พ.ศ.)"** (`modalYearFinish`) เพิ่มเติมเข้าไปในหน้าต่างป๊อปอัปเดียวกัน ทำให้เจ้าหน้าที่สามารถแก้ไข ตรวจสอบ หรือย้ายปีการศึกษาจบของนักเรียนได้ทันทีในหน้าจอเดียวโดยไม่ต้องสลับเข้าสู่หน้าต่างประวัตินักเรียนปกติ
            - ปรับปรุงการรับค่าข้ามเซิร์ฟเวอร์แบบ **Double Database Update** ในเมธอด `AdminReportGraduationStatsSave` ของคอนโทรลเลอร์ `ConAdminReportResult.php` ให้ส่งข้อมูลบันทึกข้ามสิทธิ์และอัปเดตฟิลด์ `YearFinish` ในตาราง `tb_students` (ฐานข้อมูลวิชาการกลาง) ควบคู่ไปกับการอัปเดตรายละเอียดเส้นทางชีวิตในฐานข้อมูลทะเบียนประวัติครูและนักเรียน โดยระบบจะทำการกรองและทำความสะอาดเลขบัตรประชาชน (`stu_iden` / `StudentIDNumber`) ให้ตรงกัน 100%
            - เชื่อมต่อการนำเข้าข้อมูลและการอ้างอิงผ่าน Attribute `data-year-finish` บนปุ่มแก้ไข และผูกตรรกะ JavaScript ให้โหลดแสดงผลรวมกับแดชบอร์ดสถิติในรูปแบบ Real-time
        - **ระบบเชื่อมต่อ API ข้อมูลสถิติเด็กจบการศึกษาข้ามระบบ (Graduate Statistics REST/JSON API Endpoint):**
            - พัฒนาเส้นทางและสร้าง API Endpoint ใหม่อย่างเป็นทางการที่หน้าต่างระบุบที่อยู่ `/admin/academic/api` (รองรับการทำงานทั้งคำขอแบบ GET และ POST)
            - ตรรกะของ API ออกแบบมาให้มีความชาญฉลาดและส่งกลับข้อมูล 2 โหมดหลัก:
              1. **โหมดภาพรวมทั้งหมด (Summary All Years):** หากเรียกใช้งานโดยไม่มีการส่งพารามิเตอร์ปีการศึกษามา ระบบจะดึงข้อมูลสรุปผลสถิติภาพรวมของผู้สำเร็จการศึกษาของ**ทุกปีการศึกษาที่เรียนจบจริงแบบแยกกลุ่มปี** โดยส่งรหัสข้อมูล เช่น จำนวนผู้สำเร็จการศึกษาทั้งหมด, จำนวนคนและเปอร์เซ็นต์ของกลุ่มเรียนต่อ, กลุ่มทำงาน, กลุ่มอื่น ๆ กลับไปในรูปแบบ JSON Object
              2. **โหมดสถิติรายปีฉบับละเอียด (Yearly Detailed API Data):** หากมีการส่งพารามิเตอร์ปีการศึกษามา เช่น `?year=2568` ระบบจะเปลี่ยนกระบวนการสลับกลับมาคิวรี่ส่งรายชื่อนักเรียนที่จบในปีนั้นพร้อมสถานะและที่เรียนต่อ/ทำงานจริงกลับไปให้อย่างเต็มพิกัด เพื่อรองรับการนำไปวาด Chart หรือเรียกใช้จากบอร์ดบริหารภายนอก
        - เชื่อมโยง API และเส้นทาง POST `/GraduationStats/Save` ไปยังคอนโทรลเลอร์ เพื่อนำเข้าและอัปเดตข้อมูลตรงไปยังฟิลด์ `stu_future_education` และ `stu_career_interest` ในตาราง `skjacth_personnel.tb_students` โดยอ้างอิงและทำความสะอาดเลขบัตรประชาชน (`stu_iden`) ป้องกันความผิดพลาดของระเบียน
        - บูรณาการ JavaScript AJAX และ SweetAlert2 เพื่อทำการบันทึกอย่างรวดเร็วโดยไม่ต้องรีโหลดหน้าเว็บ (Single Page UX) พร้อมทั้งอัปเดตแผนภูมิวงกลมและตัวเลขสถิติบนแดชบอร์ดทันทีที่ทำรายการสำเร็จ
    - **การแสดงผลขั้นสูงและอินเตอร์เฟสแบบพรีเมียม (Premium UX/UI Visuals):**
        - **การ์ดสรุปและแผงสถิติรวมในแถวเดียว (Unified Stats & Chart Layout Row):**
            - ปรับปรุงการจัดวางเลย์เอาต์ (Grid System) ในหน้าจอแดชบอร์ด โดยย้าย **"สัดส่วนสถิติผู้สำเร็จการศึกษา (Doughnut Chart Card)"** จากด้านข้างตารางขึ้นมาไว้ใน**แถวเดียวกันกับแถบการ์ดสรุปสถิติด้านบน** ทำให้อินเตอร์เฟสเรียงสัดส่วนสวยงามเป็นสัดส่วนเดียวกัน 12 คอลัมน์เต็ม (ประกอบด้วย: บล็อกเลือกปีการศึกษา 3 cols, การ์ดจบการศึกษาทั้งหมด 2 cols, การ์ดศึกษาต่อ 2 cols, การ์ดทำงาน 2 cols, และการ์ดสัดส่วนโดนัท Chart 3 cols)
        - **ตารางรายชื่อกรองค้นหาขนาดเต็มรูปแบบ (Full-Width Live Search & Filter Table):**
            - เมื่อย้ายแผนภูมิต้นแบบขึ้นแถวบนแล้ว ได้ปรับเปลี่ยนความกว้างของ **"ตารางรายชื่อผู้สำเร็จการศึกษา"** ให้ขยายกว้างเต็ม 100% ของหน้าจอแดชบอร์ด (`col-12` แทน `col-xl-8`) ช่วยให้แสดงผลเลขประจำตัว ชื่อ-นามสกุล และรายละเอียดสถาบันศึกษา/ที่ทำงาน ได้ชัดเจน ไม่ต้องเบียดกันและอ่านง่ายขึ้น 200%
        - **ระบบดาวน์โหลดรายงานเป็นไฟล์ Excel / CSV แบบรองรับภาษาไทย (Universal Excel/CSV Export System):**
            - เพิ่มปุ่ม **"ส่งออก Excel"** (พร้อมไอคอนกล่องดาวน์โหลด `bx bx-download`) ในฝั่งขวาบนของตารางรายชื่อนักเรียน โดยใช้การจัดเก็บและประมวลผลข้อมูลผ่าน JavaScript DOM เพื่อส่งออกข้อมูลของปีการศึกษาที่แอดมินเลือกอยู่แบบเรียลไทม์
            - ออกแบบการส่งออกให้เคารพสถานะการกรองข้อมูลบนหน้าจอ (เช่น หากเลือกกรองเฉพาะกลุ่ม "ทำงาน" ตารางจะดึงข้อมูลเฉพาะกลุ่มทำงานออกมาใส่รายงานทันที)
            - **แก้ไขปัญหายอดนิยมเรื่องภาษาไทยเพี้ยนใน Microsoft Excel:** นำส่งรหัสสัญญาณพิเศษ **UTF-8 BOM (`\uFEFF`)** นำหน้าข้อมูลในไฟล์ ทำให้เจ้าหน้าที่แนะแนวสามารถดาวน์โหลดและเปิดไฟล์รายงานบนโปรแกรม Microsoft Excel หรือ Google Sheets ภาษาไทยได้ทันที 100% โดยอักษรภาษาไทยไม่ติดรหัสเพี้ยนหรือเป็นภาษาต่างดาว
        - **แผนภูมิรายงานแบบโดนัทไดนามิกส์ (Doughnut Chart Visualizer):** นำเข้าไลบรารี `Chart.js` ประมวลผลและวาดสัดส่วนการแจกแจงแบบวงกลมแบบไดนามิกส์ด้วยสีประจำธีม (#15a362, #ffab00) สวยงามทันสมัย พร้อมระบบเลเจนด์อธิบายความหมายแบบ HTML สะอาดตา
- **ระบบสิทธิ์พิเศษและการเข้าถึงทุกเมนูอัตโนมัติสำหรับ Superadmin (Superadmin Menu & Role Bypass System):**
    - พัฒนาโครงสร้างการอนุมัติสิทธิ์การเข้าถึงแบบอัจฉริยะ (Session-level Authorization Bypass) เพื่อให้ผู้ใช้งานที่มีระดับสถานะเป็น **`superadmin`** สามารถมองเห็นและเข้าถึงข้อมูลเมนูวิชาการทุกหน้าต่างในระบบได้ 100% เต็ม แม้จะไม่ได้ถูกกำหนดสิทธิ์รายแผนกไว้ในฐานข้อมูลก็ตาม
    - **การดำเนินการสองชั้น (Dual-layer Integration):**
        - **ชั้นควบคุมตัวกรอง (Global Request Filter):** พัฒนาระบบดักจับคำร้องขอใน `AdminAuthFilter.php` ทุกหน้าเว็บ โดยหากเซสชันของผู้นั้นมีสถานะเป็น `superadmin` ระบบจะทำการแทนค่าชุดตัวแปรสิทธิ์การมองเห็น `CheckrloesAcademic` ในตัวแปร Session ของผู้เข้าใช้ให้เปิดความสามารถของทุกกลุ่มงานทันที (งานทะเบียน, งานวัดผลฯ, งานหลักสูตร, งานวิจัย, กิจกรรมพัฒนาผู้เรียน, และงานแนะแนว)
        - **ชั้นควบคุมการเข้าสู่ระบบ (Authentication Layer):** พัฒนาระบบฝังข้อมูลลงเซสชันทันทีที่ผ่านการล็อกอิน ทั้งช่องทางปกติและช่องทาง Google Login ใน `Auth.php` ทำให้ไม่ต้องรอคำสั่งดึงซ้ำ และป้องกันปัญหาระบบหลุดสิทธิ์ระหว่างใช้งาน
- **สตรีมไลน์ส่วนติดต่อผู้ใช้งานเข้ากับธีมหลักแนะแนว (Emerald Theme Integration):**
    - เปลี่ยนการระบุสีและการเน้นหลัก (Primary Accent) ในไฟล์วิว `AdminReportEnrollMain.php` และ `AdminReportEnrollDetailStudent.php` จากโทนสีม่วง/น้ำเงินของ Sneat (`#696cff`) ไปใช้โทนสีเขียวแนะแนวพรีเมียม `#15a362` ตามที่กำหนดในกฎ `GEMINI.md` อย่างสมบูรณ์แบบ
    - อัปเดตการแสดงผลตัวเน้นแถวข้อมูลที่ถูกเลือก (Selected Row Highlight), แบดจ์แสดงสถานะ (Status Badges), ปุ่มดำเนินการพรีวิวข้อมูล, สปินเนอร์ตอนดาวน์โหลดข้อมูล (Loading Spinner) และการปรับแต่งสไตล์ CSS ของแท็บเมนูให้ประทับตราสีเขียวมรกตที่น่าดึงดูดใจเป็นระเบียบ
- **แก้ไขปัญหากลุ่มแท็บแสดงประวัติข้อมูลขาดหาย (Missing Admission History Tab Pill Bugfix):**
    - แก้ไขบักขนาดใหญ่ในหน้าประวัตินักเรียนรายบุคคล (`AdminReportEnrollDetailStudent.php`) ซึ่งมีแท็บการแสดงผลข้อมูลประวัติและสถานะการสมัครเรียน (`#navs-pills-top-history`) อยู่ในโค้ดส่วนแสดงผล (Content Area) แต่ไม่มีปุ่มลิสต์แท็บ (`<li class="nav-item">`) ในหน้าจอส่งผลให้เจ้าหน้าที่ไม่สามารถคลิกเข้าชมประวัติการจัดคัดเลือก, รอบการสมัคร, ประเภทห้องเรียน, หรือสถานะการส่งตัวมอบตัวของนักเรียนคนนั้นได้
    - เพิ่มลิสต์รายการแท็บ "ประวัติการสมัคร" สไตล์ไอคอนบ็อกซ์ซีฟลาย (`bx bx-history`) เพื่อเปิดการเข้าถึงข้อมูลที่สำคัญเหล่านี้ได้อย่างสะดวกสบาย 100%
- **บังคับใช้การแสดงผลวันเป็นปี พ.ศ. ทุกมุมมอง (Buddhist Era Date Standard - Client & Server-side):**
    - พัฒนาฟังก์ชันการแปลงรูปแบบวันที่ให้เป็นปี พ.ศ. และอักษรย่อเดือนไทยครอบคลุมทั้ง 2 ช่องทาง:
        - **ระบบผู้ให้บริการ (Server-side):** พัฒนาฟังก์ชันการแปลงวันที่ `thai_date_format` ลงใน `year_helper.php` ซึ่งโหลดใช้งานแบบอัตโนมัติ (Autoloaded Helper) เพื่อใช้อัปเดตวันเกิด (`stu_birthDay`) และฟิลด์วันที่ลงบันทึกดิบอื่น ๆ ในการรายงานระดับหน้าวิว
        - **ระบบผู้ใช้งาน (Client-side):** พัฒนาฟังก์ชันพิกเซล JavaScript `formatThaiDate` ภายในหน้าวิวหลักในการแปลงวันที่ผลลัพธ์การร้องขอ AJAX แบบเรียลไทม์ ทำให้การพรีวิวประวัติด่วนแสดงผลเป็นปี พ.ศ. เสมอเมื่อคลิกเลือกรายชื่อโดยไม่ขัดต่อเงื่อนไขกำหนด

### 📁 ไฟล์ที่เกี่ยวข้อง
- `app/Config/Routes.php` (Modified - Added GraduationStats routes)
- `app/Controllers/Admin/Academic/ConAdminReportResult.php` (Modified - Added main & data methods for GraduationStats)
- `app/Views/admin/layout/_partials/sidebar.php` (Modified - Replaced HomeVisit with GraduationStats in sidebar menu)
- `app/Views/admin/Academic/AdminReportResults/AdminReportGraduationStatsMain.php` (Created - Beautiful Emerald themed main view & charts logic)
- `app/Helpers/year_helper.php` (Modified - Added `thai_date_format` helper)
- `app/Views/admin/Academic/AdminReportResults/AdminReportEnrollMain.php` (Modified)
- `app/Views/admin/Academic/AdminReportResults/AdminReportEnrollDetailStudent.php` (Modified)
